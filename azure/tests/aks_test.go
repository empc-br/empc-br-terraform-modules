...
func TestAks(t *testing.T) {
	...
	expectedClusterName := fmt.Sprintf("empc-test-cluster-%s", random.UniqueId())
	expectedResourceGroupName := fmt.Sprintf("empc-aks-test-rg-%s", random.UniqueId())
	expectedNodeCount := 3
	...
	***/
	terraformOptions := &terraform.Options{
		...
		TerraformDir: "./fixtures/aks",
		// Variables to pass to our Terraform code using -var-file  options
		// VarFiles: []string{"./fixtures/aks/terraform.tfvars"},
	...
	assert.Equal(t, int32(3), actualCount)

	// Path to the Kubernetes resource config we will test
	kubeResourcePath, err := filepath.Abs("./fixtures/aks/nginx-deployment.yaml")
	require.NoError(t, err)
	...
	***/
	options := k8s.NewKubectlOptions("", "./fixtures/aks/kubeconfig", namespaceName)
